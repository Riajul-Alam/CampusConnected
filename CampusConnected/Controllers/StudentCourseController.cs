...
namespace CampusConnected.Controllers
{
    public class StudentCourseController : Controller
    {
        ...
        public async Task<IActionResult> Create(StudentCourse std)
        {
            ...
            if (std.Id != null)
            {
                ...
                else
                {
                    ...
                    std.StudentDeptId = studentRec.StudentId;
                    std.DepartmentName=studentRec.DepartmentName;
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            int s_id = editData.StudentId;
            var Srec = await studentDB.Students.FirstOrDefaultAsync(x=>x.Id == s_id);
            if (Srec != null) {
                if (Srec.CourseComplete != null)
                {
                    var ls = Srec.CourseComplete;
                    ViewData["PreC"] = getRegisteredCourse(ls);
                }
                else
                {
                    List<string> csList = new List<string>();
                    ViewData["PreC"] = csList;
                }
            }
            else
            {
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            int s_id = detailsData.StudentId;
            var Srec = await studentDB.Students.FirstOrDefaultAsync(x => x.Id == s_id);
            if (Srec != null)
            {
                if (Srec.CourseComplete != null)
